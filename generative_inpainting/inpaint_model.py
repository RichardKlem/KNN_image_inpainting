...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, config=None, reuse=False,
        ...
        # two stage network
        cnum = 32
        with tf.compat.v1.variable_scope(name, reuse=reuse), arg_scope([gen_conv, gen_deconv], training=training,
                                                                       padding=padding):
            # stage1
    ...
    def build_wgan_discriminator(self, batch_local, batch_edges, batch_global,
                                 reuse=False, training=True):
        with tf.compat.v1.variable_scope('discriminator', reuse=reuse):
            dlocal = self.build_wgan_local_discriminator(
                batch_local, reuse=reuse, training=training)
            dedges = self.build_wgan_edges_discriminator(
                batch_edges, reuse=reuse, training=training)
            ...
            dout_edges = tf.compat.v1.layers.dense(dedges, 1, name='dout_edges_fc')
            dout_global = tf.compat.v1.layers.dense(dglobal, 1, name='dout_global_fc')
            return dout_local, dout_edges, dout_global
    ...
    def build_graph_with_losses(self, batch_data, config, training=True,
        ...
        # edges
        edges_patch_batch_pos_neg = tf.concat([edges_patch_batch_pos, edges_patch_batch_complete], axis=0)
        ...
        if config.GAN == 'wgan_gp':
            # seperate gan
            pos_neg_local, pos_neg_edges, pos_neg_global = self.build_wgan_discriminator(
                local_patch_batch_pos_neg, edges_patch_batch_pos_neg, batch_pos_neg, training=training, reuse=reuse
            )
            pos_local, neg_local = tf.split(pos_neg_local, 2)
            pos_edges, neg_edges = tf.split(pos_neg_edges, 2)
            pos_global, neg_global = tf.split(pos_neg_global, 2)
            # wgan loss
            g_loss_local, d_loss_local = gan_wgan_loss(pos_local, neg_local, name='gan/local_gan')
            g_loss_edges, d_loss_edges = gan_wgan_loss(pos_edges, neg_edges, name='gan/edges_gan')
            g_loss_global, d_loss_global = gan_wgan_loss(pos_global, neg_global, name='gan/global_gan')
            losses['g_loss'] = config.GLOBAL_WGAN_LOSS_ALPHA * g_loss_global + g_loss_edges + g_loss_local
            losses['d_loss'] = d_loss_global + d_loss_edges + d_loss_local
            # gp
            interpolates_local = random_interpolates(local_patch_batch_pos, local_patch_batch_complete)
            interpolates_edges = random_interpolates(edges_patch_batch_pos, edges_patch_batch_complete)
            interpolates_global = random_interpolates(batch_pos, batch_complete)
            dout_local, dout_edges, dout_global = self.build_wgan_discriminator(
                interpolates_local, interpolates_edges, interpolates_global, reuse=True)
            # apply penalty
            penalty_local = gradients_penalty(interpolates_local, dout_local, mask=local_patch_mask)
            penalty_edges = gradients_penalty(interpolates_edges, dout_edges, mask=edges_patch_mask)
            penalty_global = gradients_penalty(interpolates_global, dout_global, mask=mask)
            # WGAN_GP_LAMBDA default set to 10
            losses['gp_loss'] = config.WGAN_GP_LAMBDA * (penalty_local + penalty_edges + penalty_global)
            ...
            if summary and not config.PRETRAIN_COARSE_NETWORK:
                ...
                gradients_summary(g_loss_edges, batch_predicted, name='g_loss_edges')
                ...
                scalar_summary('convergence/edges_d_loss', d_loss_edges)
                ...
                scalar_summary('gan_wgan_loss/gp_penalty_edges', penalty_edges)
