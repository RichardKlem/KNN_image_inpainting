...
def init_parser():
    ...
    parser.add_argument('--remove-undecided', default=False, action='store_true',
                        help='If passed, then images with the same amount of votes won\'t be counted.')
    parser.add_argument('--in-images', default=False, action='store_true',
                        help='The statistics will be counted on images, not votes.')
    return parser
...
if __name__ == '__main__':
    ...
    if args.in_images:
        ...
        print("Baseline better images:             %d (%.2f %%)" % (
            baseline_wins_cnt, (baseline_wins_cnt / all_images_cnt * 100)))
        print("Edge discriminator better images:   %d (%.2f %%)" % (
            modified_wins_cnt, (modified_wins_cnt / all_images_cnt * 100)))
    else:
